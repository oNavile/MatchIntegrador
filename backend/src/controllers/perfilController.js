  ...
  try {
    ...
    if (tipo === 'empresa') {
      ...
      const [[a]] = await db.execute(
        `SELECT a.* FROM admins a WHERE a.usuario_id = ?`, [id]
      );
      perfil = a;
    }
...
const editarPerfil = async (req, res) => {
  ...
  try {
    ...
    if (tipo === 'empresa') {
      ...
      // NOVO: email adicionado (sem remover nada)
      const {
        nome,
        email,
        descricao,
    ...
    [id]);
      ...
      if (email) {
        ...
        // NOVO: atualiza email na tabela usuarios
        await conn.execute(
          `UPDATE usuarios
           SET email = ?
           WHERE id = ?`,
      ...
      if (palavras_chave) {
        ...
        for (const p of pcs.slice(0, 4)) {
          if (p && p.trim()) {
            await conn.execute(
              `INSERT INTO palavras_chave
               (entidade_tipo, entidade_id, palavra)
               VALUES ('empresa', ?, ?)`,
...
const verCandidato = async (req, res) => {
  try {
    ...
    const [[c]] = await db.execute(
      `SELECT c.nome, c.email, c.idade, c.telefone, c.descricao, c.arquivo,
       ...
       LEFT JOIN palavras_chave pc
         ON pc.entidade_tipo = 'candidato'
        AND pc.entidade_id = c.id
       WHERE c.id = ?
       GROUP BY c.id`,
      [id]
    );
    ...
    c.palavras_chave = c.palavras_chave
      ? c.palavras_chave.split(',')
      : [];
